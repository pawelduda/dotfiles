...
class fzf_select(Command):
    ...
    def execute(self):
        ...
        command="find -L " + self.arg(1) + " \( -path '*/\.*' -o -fstype 'dev' -o -fstype 'proc' \) -prune \
            2> /dev/null | fzf +m"
...
#         process = subprocess.Popen(['fdfind', '--hidden', '--no-ignore-vcs', '--color=always', result_sep['arg'], depth, target],
